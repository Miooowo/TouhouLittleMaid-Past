...
### 相对 1.20.1 的缺口（阶段 3 不做）

| 任务 id | 原因 |
| --- | --- |
| `sugar_cane` / `melon` / `cocoa` | 1.20.1 有独立任务；本阶段只做原版主作物 farm |
| `honey` | 1.7.10 无蜂巢/蜂蜜 |
| `snow` | 未实现 |
| `feed` | 喂主人食物，未实现 |
| `fishing` / `extinguishing` | 未实现（灭火偏阶段 4） |
| `ranged_attack` / `crossbow_attack` / `danmaku_attack` / `trident_attack` | 战斗扩展，阶段 4 |
| `board_games` | 棋类，阶段 5+ |
| 堆肥 / 砍树 | **1.20.1 任务表没有对应 id**，不是同级任务 |

其它差距：无 Brain/日程表；farm 无 Farmers Delight / 特殊作物 handler；火把不读主人副手（1.7.10 无副手）。
...
3. **任务**：耕作、剪毛、火把、挤奶、喂动物、除草。（已完成，待 push）
4. **战斗扩展**：弓、弹幕、灭火、武器损坏。
...
- 阶段 1 / 阶段 2 已 push；阶段 3 按用户要求提交本批基础工作任务。
