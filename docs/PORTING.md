...
- 任务表：全部 1.20.1 id 已登记（阶段 1 仅 `idle` / `attack` 生效）
- 祭坛单方块 + 核心物品占位
- 首次提交：`ebbda49` → `origin/master`（https://github.com/Miooowo/TouhouLittleMaid-Past）

## 阶段 2（已提交）

已实现，对标 1.20.1 核心循环。提交：`7e07884` → `origin/master`。

- **女仆 GUI**：主人右键打开 `GuiContainer`；显示名字、生命、当前任务；按钮切换坐下/跟随与已实现任务。潜行右键仍可坐下。
- **15 格背包**：3×5 槽可从 GUI 存取；NBT 随实体保存。1.20.1 默认小背包可见格是 6+6=12（另有手持/护甲），本移植按既有骨架 15 格，不拆装备栏。
- **硬盘收纳**：空手盘右键女仆写入 `MaidInfo` NBT 并变成「已存储」；对地放出并校验 `OwnerUUID`。初始硬盘召唤后变成空盘（与 1.20.1 一致）。
- **墓碑**：死亡在脚下生成不可破坏方块实体，收走背包并不掉落。主人右键取物（潜行强制扔出）；御币复活女仆。1.20.1 墓碑是实体、取物后消失、复活走胶片/神龛——此处用方块实体，并提供御币复活作为阶段 2 核心路径。
- 网络：`SimpleNetworkWrapper`（`PacketMaidGuiAction`），不做 1.20 payload。

## 阶段 3（已提交）

基础世界交互任务已用 1.7.10 `EntityAI` 落地（服务端执行，GUI/聊天显示当前任务 id）。物品优先从女仆 15 格背包取用。工作任务期间暂停跟随。

| 任务 id | 类 | 行为 |
| --- | --- | --- |
| `farm` | `EntityAIMaidFarm` | 收获成熟小麦/胡萝卜/土豆（meta≥7），耕地（需锄），补种同种作物 |
| `shears` | `EntityAIMaidShears` | 剪可剪的羊，羊毛进背包，剪刀损耗 |
| `torch` | `EntityAIMaidTorch` | 光照 &lt; 8 处放火把；先女仆背包，再附近主人背包 |
| `milk` | `EntityAIMaidMilk` | 空桶挤成年牛，产出奶桶 |
| `feed_animal` | `EntityAIMaidFeedAnimal` | 用对应繁殖食物喂成年未恋爱动物 |
| `grass` | `EntityAIMaidGrass` | 打掉草、花、双层植物、枯灌木并捡拾 |

切换：GUI「任务」按钮 / 御币走 `MaidTask.nextImplemented()`，已包含上表 + `idle` / `attack`。

提交：`d388891` → `origin/master`。

## 阶段 3 补遗（已提交）

其余 1.20.1 世界任务已用 `EntityAI` 落地，接入同一任务循环；工作期间暂停跟随。

| 任务 id | 类 | 行为 |
| --- | --- | --- |
| `sugar_cane` | `EntityAIMaidSugarCane` | 收高于根的甘蔗并补种（土/草/沙 + 邻水） |
| `melon` | `EntityAIMaidMelon` | 收紧邻茎的西瓜/南瓜；**不补种**（与 1.20.1 一致） |
| `cocoa` | `EntityAIMaidCocoa` | 收成熟可可（age≥2），补种原位；也可种在丛林原木侧面 |
| `snow` | `EntityAIMaidSnow` | 铲雪层；有铲子才掉雪球并损耗铲子 |
| `feed` | `EntityAIMaidFeed` | 从 15 格背包喂主人食物；奶桶解负面效果；金苹果在半血以下优先 |
| `fishing` | `EntityAIMaidFishing` | 岸边持鱼竿定时出鱼/杂物（无 `MaidFishingHook` 实体） |
| `extinguishing` | `EntityAIMaidExtinguish` | 水桶代替灭火器：浇灭 2×1×2 着火方块与着火生物 |
...
| --- | --- |
| `honey` | 1.7.10 无蜂巢/蜂蜜 |
| `ranged_attack` / `crossbow_attack` / `danmaku_attack` / `trident_attack` | 战斗扩展，阶段 4 |
| `board_games` | 棋类，阶段 5+ |
| 堆肥 / 砍树 | **1.20.1 任务表没有对应 id** |

其它差距：无 Brain/日程表；farm 无 Farmers Delight / 特殊作物 handler；火把不读主人副手；钓鱼无浮漂实体与原版钓饵表；灭火无专用灭火器/喷射实体（用水桶）；喂食无蜂蜜瓶。
...
1. **骨架**：工程编译 + 女仆能生成、渲染、跟随/待机/近战。（已完成并 push）
2. **生存循环**：GUI、背包、硬盘收纳、墓碑。（已完成并 push：`7e07884`）
3. **任务**：基础工作任务已 push（`d388891`）；甘蔗/瓜/可可/铲雪/喂主人/钓鱼/灭火待本提交 push。
4. **战斗扩展**：弓、弹幕、武器损坏。
...
- 阶段 1–3 基础任务已 push；阶段 3 补遗按用户要求一并提交。
